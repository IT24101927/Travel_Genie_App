...
const hotelValidation = [
  ...
  body('location').optional({ checkFalsy: true }).trim().notEmpty().withMessage('Location cannot be empty'),
...
const updateHotelValidation = [
  body('name').optional({ checkFalsy: true }).trim().notEmpty().withMessage('Hotel name cannot be empty'),
  body('location').optional({ checkFalsy: true }).trim().notEmpty().withMessage('Location cannot be empty'),
  body('address_text').optional().trim(),
  body('price_per_night').optional().isFloat({ min: 0 }).withMessage('price_per_night must be >= 0'),
  body('rating').optional().isFloat({ min: 0, max: 5 }).withMessage('rating must be between 0 and 5'),
